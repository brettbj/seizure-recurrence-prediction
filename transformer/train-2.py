...
training_round = 3
split_var = [f'train[50%:75%]', 'train[-80:]']
...
train_dataset, test_dataset = load_dataset("text", 
                                           data_files= f"{base_path}/notes_*.txt",
                                           split=split_var,
...
# checkpoint = "checkpoint-136000"
# model = AutoModelForMaskedLM.from_pretrained(f"{output_comb_dir}/{training_round}/{checkpoint}")
model = AutoModelForMaskedLM.from_pretrained(f"{output_comb_dir}/{training_round-1}")
